...
class TestInstantiateTokenizer:
    ...
    @pytest.mark.parametrize("max_seq_len, expected_max_seq_len", [
        ("null", None),
        ("2341", 2341),
    ])
    @pytest.mark.parametrize("tokenizer_name, extra_config", [
        (
            "torchtune.models.llama2.llama2_tokenizer",
            f"  path: {str(ASSETS / "m.model")}"
        ),
        (
            "torchtune.models.llama3.llama3_tokenizer",
            f"  path: {str(ASSETS / "tiktoken_small.model")}"
        ),
        (
            "torchtune.models.qwen2.qwen2_tokenizer",
            (
              f"  path: {str(ASSETS / "tiny_bpe_vocab.json")}{os.linesep}"
              f"  merges_file: {str(ASSETS / "tiny_bpe_merges.txt")}"
            )
        ),
    ])
    def test_tokenizer_null(self, tokenizer_name, extra_config, max_seq_len, expected_max_seq_len,):
        s = dedent(f"""
        tokenizer:
